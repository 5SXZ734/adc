...
static void nprint(const char *fmt, ...)
{
	(void)fmt;
}

//WINE_DEFAULT_DEBUG_CHANNEL(msvcrt);
#define ERR	nprint//printf
#define WARN	nprint//printf
#define TRACE
#define lstrcpynA	strncpy
#define debugstr_a(arg)	arg
                              ...
                              struct array* pmt_ref, BOOL in_args)
{
    ...
    case '$':
        ...
        case '$':
			...
			else if (*sym->current == 'Q')//rval reference
			{
				sym->current++;
				if (!get_modified_type(ct, sym, pmt_ref, dt, in_args)) goto done;
			}
	...
	/*case '@':
		...
		{
			sym->current++;
			ct->left = str_printf(sym, "`RTTI Type Descriptor'");
			break;
		}*/
    default :
        ERR("Unknown type %c\n", dt);
    ...
    if (*sym->current == '?' && (sym->current[1] != '$' || sym->current[2] == '?'))
    {
        ...
        case '_':
            ...
            case 'R':
                ...
                case '0':
                    {
                        struct datatype_t       ct0, ct;
                        struct array pmt;

                        sym->current++;
                        str_array_init(&pmt);
                        demangle_datatype(sym, &ct0, &pmt, FALSE);
						if (!demangle_datatype(sym, &ct, NULL, FALSE))
						{
							if (*sym->current == '8')//my
							{
								function_name = str_printf(sym, "%s%s `RTTI Type Descriptor'", ct0.left, ct0.right);
								do_after = 4;
								break;
							}
							goto done;
						}
                        function_name = str_printf(sym, "%s%s `RTTI Type Descriptor'", ct.left, ct.right);
                        sym->current--;
                    }
                    break;
                case '1':
