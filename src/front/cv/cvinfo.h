...
//
    ...
    LF_UDT_MOD_SRC_LINE = 0x1607,    // module, source and line on where an UDT is defined
        ...
        LF_CLASS_WTF     = 0x1608,      // UNDOCUMENTED!
        LF_STRUCTURE_WTF = 0x1609,      // UNDOCUMENTED!
...
inline uint32_t CVCompressData(
    uint32_t  iLen,       // [IN]  given uncompressed data
    void *  pDataOut)   // [OUT] buffer for the compressed data
{
    uint8_t *pBytes = reinterpret_cast<uint8_t *>(pDataOut);
    ...
    if (iLen <= 0x3FFF) {
        *pBytes     = uint8_t((iLen >> 8) | 0x80);
        *(pBytes+1) = uint8_t(iLen & 0xff);
        return 2;
    }

    if (iLen <= 0x1FFFFFFF) {
        *pBytes     = uint8_t((iLen >> 24) | 0xC0);
        *(pBytes+1) = uint8_t((iLen >> 16) & 0xff);
        *(pBytes+2) = uint8_t((iLen >> 8)  & 0xff);
        *(pBytes+3) = uint8_t(iLen & 0xff);
        return 4;
    }
...
inline uint32_t DecodeSignedInt32(uint32_t input)
{
