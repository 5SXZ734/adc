...
public:
	...
	std::pair<iterator, bool> insert_unique(value_type const &value)
	{
		iterator lb = lower_bound(value.first);

		//get_comparator_()(key, get_key_(where)
		if (lb != end() && !(get_comparator_()(value.first, lb->first)))
		{
			// key already exists
		...
		// add it to the map
		check_max_size_();
		return std::make_pair(iterator(sbt_insert_hint_(lb.node, sbt_create_node_(value))), true);
		//insert(lb, sbt_create_node_(value));    // Use lb as a hint to insert,
